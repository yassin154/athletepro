...
def export_excel():
    ...
    """)
    all_obj = q(conn, """
        SELECT o.*, a.nom_prenom, a.numero_licence, u.full_name as coach_name
        FROM objectifs o
        JOIN athletes a ON o.athlete_id=a.id
        JOIN users u ON a.coach_id=u.id
        WHERE o.statut='validé'
        ORDER BY a.nom_prenom, o.epreuve
    """)
    all_res = q(conn, """
        SELECT r.*, a.nom_prenom, a.numero_licence, u.full_name as coach_name
        FROM resultats r
        JOIN athletes a ON r.athlete_id=a.id
        JOIN users u ON a.coach_id=u.id
        WHERE r.statut='validé'
        ORDER BY a.nom_prenom, r.date_competition
    """)
    conn.close()
    ...
    ws1 = wb.active
    ws1.title = "Athlètes"
    headers1 = ['CRF/INA', 'Entraineur', 'Nom', 'Licence', 'Catégorie', 'Sexe', 'Spécialité', 'Épreuves', 'Club', 'Statut']
    for col, h in enumerate(headers1, 1):
        cell = ws1.cell(row=1, column=col, value=h)
        cell.font = Font(bold=True, color='FFFFFF')
        cell.fill = PatternFill(fill_type='solid', fgColor='003366')
    for row, a in enumerate(all_athletes, 2):
        ws1.append([a['centre'], a['coach_name'], a['nom_prenom'], a['numero_licence'],
                    a['categorie'], a['sexe'], a['specialite'], a['epreuves'], a['club'], a['statut']])

    # Sheet 2 - Objectifs
    ws2 = wb.create_sheet("Objectifs validés")
    headers2 = ['Athlète', 'Licence', 'Entraineur', 'Épreuve', 'Saison', 'Objectif Chrono', 'Date validation']
    for col, h in enumerate(headers2, 1):
        cell = ws2.cell(row=1, column=col, value=h)
        cell.font = Font(bold=True, color='FFFFFF')
        cell.fill = PatternFill(fill_type='solid', fgColor='003366')
    for row, o in enumerate(all_obj, 2):
        ws2.append([o['nom_prenom'], o['numero_licence'], o['coach_name'],
                    o['epreuve'], o.get('saison'), o['objectif_chrono'], o['valide_le']])

    # Sheet 3 - Résultats
    ws3 = wb.create_sheet("Résultats saisis")
    headers3 = ['Athlète', 'Licence', 'Entraineur', 'Épreuve', 'Compétition', 'Lieu', 'Date', 'Performance', 'Classement']
    for col, h in enumerate(headers3, 1):
        cell = ws3.cell(row=1, column=col, value=h)
        cell.font = Font(bold=True, color='FFFFFF')
        cell.fill = PatternFill(fill_type='solid', fgColor='003366')
    for row, r in enumerate(all_res, 2):
        ws3.append([r['nom_prenom'], r['numero_licence'], r['coach_name'],
                    r['epreuve'], r['nom_competition'], r['lieu'],
                    r['date_competition'], r['performance'], r.get('classement')])
